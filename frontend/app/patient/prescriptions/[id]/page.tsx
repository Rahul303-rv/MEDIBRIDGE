    ...
    <main className="min-h-screen bg-zinc-50 p-8">
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold text-zinc-900">Prescription</h1>
          <div className="flex items-center gap-3">
            ...
            <Link href="/patient/prescriptions" className="text-sm text-zinc-500 hover:underline">← Back</Link>
          </div>
        </div>
          ...
          <CardContent className="pt-5">
            <div className="flex items-start justify-between">
              <div>
                ...
                {rx.doctor_reg_no && <p className="text-xs text-zinc-400">Reg No: {rx.doctor_reg_no}</p>}
              </div>
            ...
            <p className="text-sm text-zinc-700 mt-3">Patient: <strong>{rx.patient_name}</strong></p>
          </CardContent>
        </Card>
        ...
        <Card className="border border-zinc-200 shadow-sm">
          ...
          <CardContent>
            <p className="text-sm text-zinc-700">{rx.diagnosis}</p>
          </CardContent>
        </Card>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {rx.medicines.map((med) => (
                      <tr key={med.id} className="border-b border-zinc-50">
                        <td className="py-2 pr-4 text-zinc-900 font-medium">{med.medicine_name}</td>
                        <td className="py-2 pr-3 text-zinc-600">{med.dosage}</td>
                        ...
                        <td className="py-2 px-3 text-zinc-600 whitespace-nowrap">{MEAL_LABELS[med.meal_timing] ?? med.meal_timing}</td>
                        <td className="py-2 px-2 text-center text-zinc-600">{med.duration_days}</td>
                        <td className="py-2 pl-3 text-zinc-500 text-xs">{med.instructions || "—"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <CardContent className="space-y-2">
              {rx.tests.map((t) => (
                <div key={t.id} className="flex items-start gap-3">
                  <p className="text-sm text-zinc-800 font-medium flex-1">{t.test_name}</p>
        ...
        {surgeryRecs.length > 0 && (
          <Card className="border border-teal-200 shadow-sm bg-teal-50">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-semibold text-teal-800">Surgery Recommended</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              {surgeryRecs.map((rec) => (
                <div key={rec.id} className="bg-white rounded-lg border border-teal-100 p-4 space-y-2">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="font-semibold text-zinc-900">{rec.package_name}</p>
                      <p className="text-xs text-zinc-500">{rec.hospital_name} · {rec.surgery_type}</p>
                    </div>
                    <p className="text-lg font-bold text-teal-700 shrink-0">${rec.price_usd}</p>
                  </div>
                  {rec.notes && (
                    <p className="text-sm text-zinc-700 border-t border-zinc-100 pt-2">{rec.notes}</p>
                  )}
                  <Link
