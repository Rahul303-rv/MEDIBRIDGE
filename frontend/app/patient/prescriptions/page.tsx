      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-4">
          ...
          <div className="space-y-2">
            <div className="h-4 bg-zinc-100 dark:bg-zinc-700 rounded w-32" />
            <div className="h-3 bg-zinc-100 dark:bg-zinc-700 rounded w-44" />
            <div className="h-3 bg-zinc-100 dark:bg-zinc-700 rounded w-64" />
          </div>
        </div>
        <div className="w-16 h-8 bg-zinc-100 dark:bg-zinc-700 rounded-xl shrink-0" />
      </div>
    </div>
  );
}
...
export default function PatientPrescriptionsPage() {
  ...
  return (
    <div className="p-4 sm:p-8 space-y-6 max-w-3xl">
      ...
      {/* Header */}
      <div>
        <h1 className="text-2xl font-extrabold text-zinc-900 dark:text-white">My Prescriptions</h1>
        <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-0.5">Digital prescriptions from your consultations</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-zinc-800 rounded-2xl border border-zinc-200 dark:border-zinc-700 p-14 text-center">
          ...
          <p className="font-bold text-zinc-700 dark:text-zinc-200 text-lg">No prescriptions yet</p>
          ...
          {prescriptions.map((rx) => {
            ...
            return (
              <Link key={rx.id} href={`/patient/prescriptions/${rx.id}`}>
                <div className="bg-white dark:bg-zinc-800 rounded-2xl border-2 border-zinc-200 dark:border-zinc-600 hover:border-teal-400 dark:hover:border-teal-500 hover:shadow-md transition-all p-5 group">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-start gap-4 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="font-bold text-zinc-900 dark:text-white group-hover:text-teal-700 transition-colors">
                          {rx.doctor_name}
                        </p>
                        ...
                        <p className="text-sm text-zinc-600 dark:text-zinc-300 mt-1.5 line-clamp-2 leading-relaxed">
                          {rx.diagnosis}
                        </p>
                      </div>
                    </div>
