        ...
        <div key={i} className="flex items-center flex-1 last:flex-none">
          <div className="flex flex-col items-center">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-colors ${
              ...
              : i === current ? "bg-white dark:bg-zinc-800 border-teal-600 text-teal-600"
              : "bg-white dark:bg-zinc-800 border-zinc-300 dark:border-zinc-600 text-zinc-400 dark:text-zinc-500"
            }`}>{i < current ? "✓" : i + 1}</div>
            <span className={`text-xs mt-1 text-center ${i === current ? "text-teal-600 font-medium" : "text-zinc-400 dark:text-zinc-500"}`}>{label}</span>
          </div>
          {i < STEPS.length - 1 && (
            <div className={`flex-1 h-0.5 mb-4 mx-1 ${i < current ? "bg-teal-600" : "bg-zinc-200 dark:bg-zinc-700"}`} />
          )}
        </div>
      ))}
    </div>
    ...
    <main className="min-h-screen bg-zinc-50 dark:bg-zinc-900 p-4 sm:p-8">
      <div className="max-w-2xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm text-zinc-500 dark:text-zinc-400">Booking</p>
            <h1 className="text-xl font-bold text-zinc-900 dark:text-white">{pkg.name}</h1>
            <p className="text-sm text-zinc-500 dark:text-zinc-400">{pkg.hospital_name} · ${Number(pkg.price_usd).toLocaleString()} USD</p>
          </div>
          <Link href="/patient/surgery-bookings" className="text-sm text-zinc-500 dark:text-zinc-400 hover:underline">← My Bookings</Link>
        </div>
        ...
        {step === 0 && (
          <Card className="border border-zinc-200 shadow-sm">
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                <label className="text-xs text-zinc-600 dark:text-zinc-400 mb-1 block">Tentative Surgery Date *</label>
          ...
          <Card className="border border-zinc-200 shadow-sm">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-zinc-600 dark:text-zinc-400 mb-1 block">Passport Number *</label>
                  <input required value={travelInfo.passport_number} onChange={tiField("passport_number")} className={inputCls} />
                </div>
                <div>
                  <label className="text-xs text-zinc-600 dark:text-zinc-400 mb-1 block">Passport Country *</label>
                  <input required value={travelInfo.passport_country} onChange={tiField("passport_country")} className={inputCls} />
                </div>
                <div>
                  <label className="text-xs text-zinc-600 dark:text-zinc-400 mb-1 block">Passport Expiry *</label>
                  <input type="date" required value={travelInfo.passport_expiry} onChange={tiField("passport_expiry")} className={inputCls} />
                </div>
                <div>
                  <label className="text-xs text-zinc-600 dark:text-zinc-400 mb-1 block">Current Occupation *</label>
                  <input required value={travelInfo.current_occupation} onChange={tiField("current_occupation")} className={inputCls} />
                </div>
                <div>
                  <label className="text-xs text-zinc-600 dark:text-zinc-400 mb-1 block">Employer</label>
                  <input value={travelInfo.employer} onChange={tiField("employer")} className={inputCls} />
                </div>
                <div>
                  <label className="text-xs text-zinc-600 dark:text-zinc-400 mb-1 block">Annual Income (USD)</label>
                  <input type="number" min="0" value={travelInfo.annual_income_usd} onChange={tiField("annual_income_usd")} className={inputCls} />
                </div>
                <div>
                  <label className="text-xs text-zinc-600 dark:text-zinc-400 mb-1 block">Companions</label>
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={travelInfo.visa_required}
                  ...
                  className="rounded" />
                <span className="text-sm text-zinc-700 dark:text-zinc-300">Visa required for India</span>
              </label>
              {Number(travelInfo.companion_count) > 0 && (
                <div>
                  <label className="text-xs text-zinc-600 dark:text-zinc-400 mb-1 block">Companion Details (one per line)</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-zinc-600 dark:text-zinc-400 mb-1 block">Dietary Requirements</label>
                  <textarea rows={2} value={travelInfo.dietary_requirements} onChange={tiField("dietary_requirements")} className={inputCls} />
                </div>
                <div>
                  <label className="text-xs text-zinc-600 dark:text-zinc-400 mb-1 block">Special Needs</label>
          ...
          <Card className="border border-zinc-200 shadow-sm">
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2 text-sm">
                <p className="text-zinc-600 dark:text-zinc-400">Required: <span className="font-medium">Passport scan</span>, <span className="font-medium">Government ID</span>{needsVisa && <>, <span className="font-medium">Visa</span></>}</p>
                <p className="text-xs text-zinc-400 dark:text-zinc-500">Accepted: PDF, JPEG, PNG · Max 10 MB per file</p>
              </div>
                ...
                <div className="space-y-2">
                  {documents.map((doc) => (
                    <div key={doc.id} className="flex items-center justify-between bg-zinc-50 dark:bg-zinc-700 rounded-lg px-3 py-2">
                      <div>
                        <span className="text-sm font-medium text-zinc-700 dark:text-zinc-300 capitalize">{doc.doc_type.replace("_", " ")}</span>
        ...
        {step === 3 && (
          <Card className="border border-zinc-200 shadow-sm">
            ...
            <CardContent className="space-y-4">
              <div className="bg-zinc-50 dark:bg-zinc-700 rounded-lg p-4 space-y-3 text-sm">
                <div>
                  ...
                  <p className="text-zinc-500 dark:text-zinc-400">{pkg.hospital_name} · {pkg.hospital_city}</p>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div><p className="text-zinc-400 dark:text-zinc-500 text-xs">Tentative Date</p><p className="text-zinc-700 dark:text-zinc-300">{tentativeDate}</p></div>
                  <div><p className="text-zinc-400 dark:text-zinc-500 text-xs">Duration</p><p className="text-zinc-700 dark:text-zinc-300">{pkg.total_duration_days} days</p></div>
                  <div><p className="text-zinc-400 dark:text-zinc-500 text-xs">Passport</p><p className="text-zinc-700 dark:text-zinc-300">****{travelInfo.passport_number.slice(-4)} ({travelInfo.passport_country})</p></div>
                  <div><p className="text-zinc-400 dark:text-zinc-500 text-xs">Occupation</p><p className="text-zinc-700 dark:text-zinc-300">{travelInfo.current_occupation}</p></div>
                </div>
                <div>
                ...
                <div className="border-t border-zinc-200 dark:border-zinc-600 pt-3">
                  <p className="text-xs text-zinc-400 dark:text-zinc-500 uppercase tracking-wide">Total Amount</p>
                  <p className="text-2xl font-bold text-zinc-900 dark:text-white mt-1">${Number(pkg.price_usd).toLocaleString()} USD</p>
                </div>
              </div>
              <div className="flex gap-3">
          ...
          <Card className="border border-zinc-200 shadow-sm">
            ...
            <CardContent className="space-y-4">
              <div className="bg-teal-50 dark:bg-teal-900/20 border border-teal-200 dark:border-teal-800 rounded-lg p-4">
                <p className="text-sm text-teal-800 dark:text-teal-300 font-medium">Amount Due</p>
                <p className="text-3xl font-bold text-zinc-900 dark:text-white mt-1">${Number(pkg.price_usd).toLocaleString()}</p>
                <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">USD · {pkg.name}</p>
              </div>

              {!showPayment ? (
                <div className="space-y-3">
                  <p className="text-sm text-zinc-500 dark:text-zinc-400">This is a demo payment. No real transaction will occur.</p>
                  <div className="flex gap-3">
                ...
                <div className="border border-zinc-200 dark:border-zinc-700 rounded-lg p-4 space-y-3">
                  <p className="text-sm font-medium text-zinc-700 dark:text-zinc-300">Demo Payment Terminal</p>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="col-span-2">
                      <label className="text-xs text-zinc-500 dark:text-zinc-400 mb-1 block">Card Number</label>
                      <input defaultValue="4111 1111 1111 1111" readOnly className={`${inputCls} bg-zinc-50 text-zinc-400`} />
                    </div>
                    <div>
                      <label className="text-xs text-zinc-500 dark:text-zinc-400 mb-1 block">Expiry</label>
                      <input defaultValue="12/28" readOnly className={`${inputCls} bg-zinc-50 text-zinc-400`} />
                    </div>
                    <div>
                      <label className="text-xs text-zinc-500 dark:text-zinc-400 mb-1 block">CVV</label>
        ...
        {/* Step 5: Confirmed */}
        {step === 5 && booking && (
          <Card className="border border-emerald-200 dark:border-emerald-800 shadow-sm bg-emerald-50 dark:bg-emerald-900/20">
            <CardContent className="pt-8 pb-8 text-center space-y-4">
              <div className="text-5xl">✅</div>
              <h2 className="text-xl font-bold text-zinc-900 dark:text-white">Booking Confirmed!</h2>
              <p className="text-sm text-zinc-600 dark:text-zinc-400">
                Your surgery package has been booked. Your voucher PDF has been emailed to you.
              </p>
              {booking.coupon && (
                <div className="inline-block bg-white dark:bg-zinc-800 border border-emerald-200 dark:border-emerald-800 rounded-lg px-4 py-2">
                  <p className="text-xs text-emerald-600 dark:text-emerald-400 font-medium uppercase tracking-wide">Voucher Code</p>
                  <p className="font-mono text-lg font-bold text-emerald-700 dark:text-emerald-400">{booking.coupon.code}</p>
                </div>
              )}
              <div className="flex gap-3 justify-center flex-wrap">
                ...
                <Link href="/patient/surgery-bookings"
                  className="inline-flex items-center justify-center h-8 px-4 rounded-lg border border-zinc-200 dark:border-zinc-700 text-sm text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800">
                  View All Bookings
                </Link>
              </div>
            </CardContent>
