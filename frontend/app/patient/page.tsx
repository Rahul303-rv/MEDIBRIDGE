...
const QUICK_ACTIONS = [
  {
    ...
    icon: "🩺",
    color: "bg-teal-50 border-teal-100 hover:border-teal-300 hover:bg-teal-50/80",
  ...
  },
  {
    ...
    icon: "📋",
    color: "bg-white border-zinc-200 hover:border-teal-300 hover:bg-zinc-50",
...
function ProfileField({ label, done }: { label: string; done: boolean }) {
  return (
    <div className="flex items-center gap-2">
      <div className={`w-4 h-4 rounded-full flex items-center justify-center shrink-0 ${done ? "bg-teal-500" : "bg-zinc-200"}`}>
        {done && (
      ...
      <span className={`text-xs ${done ? "text-zinc-500 line-through" : "text-zinc-700"}`}>{label}</span>
    </div>
  );
}
    ...
    <div className="p-8 space-y-8 max-w-5xl">
      ...
      <div className="rounded-2xl border border-zinc-200 bg-white p-5 flex items-center gap-5">
        ...
        <div className="flex-1 min-w-0">
          <p className="font-semibold text-zinc-900 text-sm">Looking for a specific doctor?</p>
          <p className="text-xs text-zinc-500 mt-0.5">Browse our verified specialists and view their profiles.</p>
        </div>
        <Link
          href="/doctors"
